...
#include </home/user/sankar/vsomeip/examples/hello_world/hello_world_service.hpp>
...
struct pub_header //structure to create the data packet        
{ 
char* com_type="PUBLISHER";      //setting communication to pub/sub mode            
char* pub_data;       //variable to store the publish data            
uint32_t pub_len=1000;     //variable to store the publish data length        
}p1;
    ...
    void notify() {
       ...
       // vsomeip::byte_t its_data[100]= {' ',' '};
        vsomeip::byte_t its_data[1000];
        uint32_t its_size = 1;
        int i;
        ...
        while (running_) {
            ...
            std::ifstream MyReadFile("/home/user/sankar/vsomeip/examples/test_publisher_data.txt");
            std::string hari;
            int mjb = 0;
            while (is_offered_ && running_) {
                mjb++;
                ...
                // p1.pub_data="loading the publisher data ...";
                // for(i = 0; p1.pub_data[i] !='\0'; i++){
                    
                //     its_data[i] = p1.pub_data[i];
                // }
                // its_data[i] = '\0';


                getline(MyReadFile, hari);
                    //std::cout<<hari;
                for(i = 0; hari[i] !='\0'; i++){
                ...
                {
                    std::lock_guard<std::mutex> its_lock(payload_mutex_);
                    payload_->set_data(its_data, p1.pub_len);

                    //p1.com_type="PUBLISHER";

                   // p1.pub_data="intiating publisher communication";
                   // p1.pub_len=its_size;// std::cout << "Setting event (Length=" << std::dec << its_size << ")." << std::endl;                   
                    //[]std::cout <<"------------------------------------------------------------"<<std::endl;

                   // std::cout <<"Setting event-publisher data (Comm Type = " << p1.com_type << " publisher data = "<< p1.pub_data << "  Publisher data Length= " << p1.pub_len << std::endl;
                   // std::cout <<"------------------------------------------------------------"<<std::endl;

    
                    // std::cout <<"Setting event-publisher data (Comm Type = " << p1.com_type  << "  Publisher data Length= " << p1.pub_len << std::endl;
                    // std::cout <<""<<std::endl;
                    // std::cout << "print count ------ "<<mjb <<"    "<< running_ << std::endl;
                    //std::cout << "Setting event (Length=" << std::dec << 10 << ")." << std::endl;
                    app_->notify(SAMPLE_SERVICE_ID, SAMPLE_INSTANCE_ID, SAMPLE_EVENT_ID, payload_);
                }
                // std::cout << is_offered_<<"    "<< running_ << std::endl;
